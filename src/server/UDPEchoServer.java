...
public class UDPEchoServer {
    ...
    public static void start(int portNumber, int bufferSize)
            throws UDPEchoServerException {
    ...
    private static void run() throws IOException {
        ...
        while (true) {
            received = receive();
            if (dataIsUnderSize(received)) reply(received, scanner);
            else {
    ...
    private static void reply(DatagramPacket received, Scanner scanner) throws IOException {
        String clientMessage = new String(received.getData(), 0, received.getLength()).trim();
        System.out.printf("Mensagem: '%s' de '%s'\n", clientMessage, received.getAddress().getHostAddress());
        System.out.print("Escreva uma resposta: ");
        String serverResponse = scanner.nextLine();
        byte[] responseBytes = serverResponse.getBytes();
        DatagramPacket reply = new DatagramPacket(responseBytes, responseBytes.length, received.getAddress(), received.getPort());
