...
public class UDPClient {
	...
	private static int SERVER_PORT = 3004;
	...
	public static void main(String[] args) {
		...
		while (!stringMessage.equals(KEY_TO_EXIT)) {
			System.out.printf("Escreva uma mensagem (%s para sair): ",
					KEY_TO_EXIT);
			...
			if (!stringMessage.equals("q")) {
				try {
					...
					System.out.printf("Resposta do servidor: %s\n", new String(
							datagramPacketForResponse.getData()));
	...
	private static void closeOpenedResources(DatagramSocket datagramSocket,
			Scanner reader) {
